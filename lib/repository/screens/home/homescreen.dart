...
// ✅ Added WidgetsBindingObserver for midnight reset detection
class _ZyraHomePageState extends State<ZyraHomePage> with WidgetsBindingObserver {
  ...
  // ✅ Manual refresh after returning from any screen
  ...
  // ✅ Detects date change and re-subscribes stream to new day's doc
  void _checkDateAndReinitialize() {
    String today = DateFormat('yyyy-MM-dd').format(DateTime.now());
    if (today != _lastTrackedDate) {
      debugPrint('📅 Date changed to $today — reinitializing wellness stream');
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        if (waterGlasses < 8) {
                          // ✅ Capture value before async gap to avoid race condition
  ...
  // ✅ Safe cast + mounted guard + error reconnect + cancels old sub first
  ...
  // ✅ Fires when app resumes from background — catches midnight date changes
  @override
            ...
            children: [
              GestureDetector(
                onTap: () async {
                  ...
                  await _refreshWellnessData(); // ✅ refresh symptoms on return
                },
                child: Container(
                  height: 95,
                  decoration: BoxDecoration(
