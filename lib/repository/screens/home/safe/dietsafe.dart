...
// import 'package:zyra_final/domain/services/csv_upload_service.dart';
...
//                                 ElevatedButton(
//                                   onPressed: () async {
//                                     await CsvUploadService.uploadDietCSV(
//                                       startIndex: 1,
//                                       limit: 50,
//                                     );
//                                   },
//                                   child: Text("Upload Foods Batch 1"),
//                                 ),
//                               ],
//                             ),
//                           ),
...
class _DietScreenState extends State<DietScreen>
  ...
  // ── LOAD IMAGES FROM FIREBASE (already stored in todayMeal map) ─────────
  void _loadImages() {
    setState(() {
      _breakfastImageUrl = DietService.getMealImage(todayMeal, 'Breakfast');
      _lunchImageUrl     = DietService.getMealImage(todayMeal, 'Lunch');
      _dinnerImageUrl    = DietService.getMealImage(todayMeal, 'Dinner');
      _imageLoading      = false;
    });
  }
  ...
  String? _currentImageUrl() {
    switch (_selectedMeal) {
      case 0: return _breakfastImageUrl;
      case 1: return _lunchImageUrl;
      case 2: return _dinnerImageUrl;
      default: return null;
    }
  }
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: const Icon(Icons.refresh_rounded,
                color: Color(0xFF95B289), size: 26),
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: const Center(
        child: Icon(Icons.restaurant,
            color: Color(0xFF95B289), size: 56),
      ),
    );
  }
  ...
  Widget _mealDetailCard() {
    ...
    // Parse preparation into numbered steps (separated by ' | ')
    final rawPrep = meal['preparation'] ?? '';
    final steps = rawPrep
        .split(' | ')
        .map((s) => s.trim())
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
              _miniMacro('Protein', meal['protein'] ?? '0',
                  const Color(0xFF81C784)),
              _miniMacro(
                  'Carbs', meal['carbs'] ?? '0', const Color(0xFFFFB74D)),
              _miniMacro(
                  'Fat', meal['fat'] ?? '0', const Color(0xFFE57373)),
            ],
          ),
          const Divider(height: 24, color: Color(0xFFE0E0E0)),
  ...
  Widget _macrosCard() {
    final fat = int.tryParse(todayMeal['TotalFat_g'] ?? '0') ?? 0;
    final protein =
        int.tryParse(todayMeal['TotalProtein_g'] ?? '0') ?? 0;
    final carbs =
        int.tryParse(todayMeal['TotalCarbs_g'] ?? '0') ?? 0;
    final total = (fat + protein + carbs).toDouble();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _macroBar('Protein', protein, const Color(0xFF81C784),
                  total),
              _macroBar(
                  'Carbs', carbs, const Color(0xFFFFD54F), total),
              _macroBar(
                  'Fat', fat, const Color(0xFFE57373), total),
            ],
          ),
        ],
      ),
  ...
  // ── UPLOAD SECTION (temporary — remove after all batches uploaded) ─────────
  int _batchIndex = 1; // 1-based, always clamped to 1–4
  bool _uploading = false;
  String _uploadStatus = '';

  // Batch definitions
  static const List<Map<String, Object>> _batches = [
    {'label': 'Batch 1 (rows 1-50)',    'start': 1,   'limit': 50},
    {'label': 'Batch 2 (rows 51-100)',  'start': 51,  'limit': 50},
    {'label': 'Batch 3 (rows 101-150)', 'start': 101, 'limit': 50},
    {'label': 'Batch 4 (rows 151-200)', 'start': 151, 'limit': 50},
  ];

  Widget _uploadSection() {
    final batches = _batches;
    final safeIdx = (_batchIndex - 1).clamp(0, batches.length - 1);
    final currentBatch = batches[safeIdx];
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text(
            '🔧 Admin: Upload Meals to Firebase',
            style: TextStyle(
              fontFamily: 'Outfit',
              fontWeight: FontWeight.bold,
              fontSize: 14,
          ...
          const SizedBox(height: 4),
          const Text(
            'Run one batch per day (150 API calls/day limit)',
            style: TextStyle(fontFamily: 'Outfit', fontSize: 11, color: Colors.black54),
          ),
          ...
          Wrap(
            ...
            children: List.generate(batches.length, (i) {
              final selected = _batchIndex == i + 1;
              return GestureDetector(
                onTap: _uploading ? null : () => setState(() => _batchIndex = i + 1),
                child: Container(
            ...
            child: ElevatedButton(
              onPressed: _uploading
                  ...
                  : () async {
                      setState(() {
                        _uploading = true;
                        _uploadStatus = 'Uploading ${currentBatch["label"]}...';
                      });
                      try {
                        await CsvUploadService.uploadDietCSV(
                          startIndex: currentBatch['start'] as int,
                          limit: currentBatch['limit'] as int,
                        );
                        setState(() {
                          _uploadStatus = '✅ ${currentBatch["label"]} uploaded!';
